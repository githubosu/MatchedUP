...
@property (strong, nonatomic) NSMutableArray *avaliableChatRoomsArray;
...
-(NSMutableArray *)avaliableChatRoomsArray
{
    if (!_avaliableChatRoomsArray) {
        _avaliableChatRoomsArray = [[NSMutableArray alloc] init];
    }
    return _avaliableChatRoomsArray;
}
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    ...
    chatVC.chatRoom = [self.avaliableChatRoomsArray objectAtIndex:indexPath.row];
...
- (void)updateAvaliableChatRooms
{
    ...
    PFQuery *queryCombined = [PFQuery orQueryWithSubqueries:@[queryInverse, query]];
    [queryCombined includeKey:@"chat"];
    [queryCombined includeKey:@"user1"];
    [queryCombined includeKey:@"user2"];
    [queryCombined findObjectsInBackgroundWithBlock:^(NSArray *objects, NSError *error) {
        if (!error) {
            [self.avaliableChatRoomsArray removeAllObjects];
            self.avaliableChatRoomsArray = [objects mutableCopy];
...
{
    return [self.avaliableChatRoomsArray count];
}
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    PFObject *chatRoom = [self.avaliableChatRoomsArray objectAtIndex:indexPath.row];
